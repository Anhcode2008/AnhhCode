...
def kramer(content: str, key: int) -> str:
    ...
    _final_content_ = fr"""class TAT07():
#ByAnhCode#   	
...
ANHCODE(_n1_=False,_n2_=False,_sparkle='''{content}''')""".strip().replace("_n1_",n_1.removeprefix("self.")).replace("_n2_",n_2.removeprefix("self.")).replace("_n3_",n_3.removeprefix("self.")).replace("_n4_",n_4.removeprefix("self.")).replace("_n5_",n_5).replace("_n6_",n_6).replace("_n7_",n_7).replace("_n8_",n_8).replace("_n9_",n_9.removeprefix("self.")).replace("_n10_",n_10.removeprefix("self.")).replace("_n12_",n_12.removeprefix("self."))
    return _final_content_

System.Clear()
System.Title("ANHCODETOOL")
System.Size(120, 45)
...
║▇▇▇▇◣▇◢▇▇▇▇║
╚═══════════╝
Copyright By: @anhhcode Version : 2.0                                           
──────────────────────────────────────────            
[</>] Box Tele: https://t.me/shareanhcode
[</>] Web CLTX: https://anhcode.click
[</>] ADMIN: AnhhCode
[</>] NOTE: ENCODE - CHƯA PHẢI LÀ BÁ!
──────────────────────────────────────────                                                
'''[1:]
...
def main():
    ...
    _file = Write.Input("[</>] Nhập Tên File Python: ", Colors.red_to_yellow, interval=0.005)

    if not _file.strip() or not isfile(_file):
        Colorate.Error("Lỗi Không Tìm Thấy File! Hãy Để File Cùng Thư Mục")
        return
    ...
    print()
    key = Write.Input("[</>] Khều Donate [ VCB - 1042469410 ] [Nhấn ENTER]", Colors.red_to_yellow, interval=0.005)
    ...
    file = file.removesuffix(".py") + "_enc.py" 
...
# Making the final content..."""))
    ...
    print(Colorate.Diagonal(Colors.red_to_yellow, f"""[</>] ENCODE THÀNH CÔNG!"""))
    ...
    print(Colorate.Diagonal(Colors.red_to_yellow, """[</>] CẢM ƠN BẠN ĐÃ SỬ DỤNG! CÓ LÒNG HÃY LỘC ADMIN TÍ NHÉ HIHI"""))

    print('\n')

    Write.Input("Vui Lòng Enter Để Lưu File!", Colors.red_to_yellow, interval=0.005)
    return exit()
