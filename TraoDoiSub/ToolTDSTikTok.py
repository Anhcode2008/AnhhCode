...
def delay(dl):
  try:
    for i in range(dl, -1, -1):
       print(f'{vang}[{trang}AN ORIN{vang}][{trang}'+str(i)+vang+']           ',end='\r')
       sleep(1)
  except:
     sleep(dl)
     print(dl,end='\r')
...
def main():
	...
	while True:
		...
		if not os.path.exists('configtds.txt'):
			token = input(f'{anhcode}{luc}Nhập Access_Token TDS: {vang}')
...
#while True:
	...
	banner()
	print(f'{anhcode}{luc}Tên Tài Khoản: {vang}{user} ')
	print(f'{anhcode}{luc}Xu Hiện Tại: {vang}{xu}')
	print(f'{anhcode}{luc}Xu Bị Phạt: {vang}{xudie} ')
	while True:
		ntool=0
		bongoc(14)
		print(f'{anhcode}{luc}Nhập {red}[{vang}1{red}] {luc}Để Chạy Nhiệm Vụ Tim')
		print(f'{anhcode}{luc}Nhập {red}[{vang}2{red}] {luc}Để Chạy Nhiệm Vụ Follow')
		print(f'{anhcode}{luc}Nhập {red}[{vang}3{red}] {luc}Để Chạy Nhiệm Vụ Follow Tiktok Now')
		nhiem_vu=input(f'{anhcode}{luc}Nhập Số Để Chạy Nhiệm Vụ: {vang}')
		dl = int(input(f'{anhcode}{luc}Nhập Delay: {vang}'))
		while True:
			if ntool == 2:
				break
			ntool = 0
			bongoc(14)
			nv_nhan=int(input(f'{anhcode}{luc}Sau Bao Nhiêu Nhiệm Vụ Thì Nhận Xu: {vang}'))
			if nv_nhan < 8:
			...
			if nv_nhan > 15:
				...
				continue
			user_cau_hinh=input(f'{anhcode}{luc}Nhập User Name Tik Tok Cần Cấu Hình: {vang}')
								...
								if cache != True:
									...
									if dem % nv_nhan == 0:
										...
										if nhan == 0:
											...
											print(f'{anhcode}{luc}Nhập {red}[{vang}1{red}] {luc}Để Thay Nhiệm Vụ ')
											print(f'{anhcode}{luc}Nhập {red}[{vang}2{red}] {luc}Thay Acc Tiktok ')
											print(f'{anhcode}{luc}Nhấn {red}[{vang}Enter{red}] {luc}Để Tiếp Tục')
											chon=input(f'{anhcode}{luc}Nhập {trang}===>: {vang}')
											if chon == '1':
												ntool=2
												break
											elif chon =='2':
